...
def main(local_rank, args):
    ...
    if args.do_train:
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if nb_tr_steps % args.gradient_accumulation_steps == 0:
                    ...
                    scheduler.step()
                    global_step += 1
                    if rank == 0 and local_rank == 0 and global_step % 50 == 0:
                        train_loss = round(tr_loss * args.gradient_accumulation_steps / (nb_tr_steps + 1), 4)
                        logger.info("step {}/{}: Train loss {}".format(
                            global_step, num_train_optimization_steps, round(train_loss, 3)))
                
                if rank == 0 and local_rank == 0 and (step == len(train_dataloader)-1 or hfai.receive_suspend_command()):
            ...
            if args.do_eval:
                ...
                # if rank == 0 and local_rank == 0 and args.save_last_checkpoints:
                #     last_output_dir = os.path.join(args.output_dir, 'checkpoint-last')
                #     if not os.path.exists(last_output_dir):
                #         os.makedirs(last_output_dir)
                #     model_to_save = model.module if hasattr(model, 'module') else model
        ...
        if rank == 0 and local_rank == 0 and args.data_num == -1:
            # tb_writer.close()
