...
python -m torch.distributed.launch --nproc_per_node ${PER_NODE_GPU} --node_rank=${RANK} --nnodes=${NODES} --master_addr=${MASTER_HOST} --master_port=${MASTER_PORT} ../run_finetune_msggen.py  \
  ...
  --output_dir ${mnt_dir}/PreViewer/saved_models_gen_codet5_simple \
  --train_filename ${mnt_dir}/Processor/data/msg-train-stop.jsonl \
  --dev_filename ${mnt_dir}/Processor/data/msg-valid-stop.jsonl \
  ...
  # --output_dir ${mnt_dir}/PreViewer/saved_models_gen_shuai \
