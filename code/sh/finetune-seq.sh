...
mnt_dir="/home/v-zhuoli1/lzzz/somework"
...
python -m torch.distributed.launch --nproc_per_node ${PER_NODE_GPU} --node_rank=${RANK} --nnodes=${NODES} --master_addr=${MASTER_HOST} --master_port=${MASTER_PORT} ../run_finetune_seq2seq.py  \
  ...
  --add_lang_ids \
  --train_epochs 40 \
  --config_name ${mnt_dir}/models/tufano \
  --tokenizer_path ${mnt_dir}/models/tufano/TokenizerModel.model \
  --model_name_or_path ${mnt_dir}/models/tufano \
  --output_dir ${mnt_dir}/PreViewer/saved_models_tufano \
  --train_filename ${mnt_dir}/baseline_data/review_tufano_aug/train.json \
  --dev_filename ${mnt_dir}/baseline_data/review_tufano_aug/valid.json \
